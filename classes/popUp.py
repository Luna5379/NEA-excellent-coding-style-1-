...
class popUp:
    ...
    def drawPopUpBase(self):
      if self.popped:
        popUpBase = pygame.Rect(196.5-self.popUpSize[0]/2, 200+self.popUpSize[1]/4,self.popUpSize[0],self.popUpSize[1])
    ...
    def userInput(self, code):
      ...
      if self.popped:
        for i in range(4):
          outline[i] = pygame.Rect(196.5-(self.popUpSize[0]/2)+22.5+75*(i), 213+self.popUpSize[1]/4+77.5,70,125)
          pygame.draw.rect(self.surface,self.outline,outline[i],border_radius = 5)
          box[i] = pygame.Rect(196.5-(self.popUpSize[0]/2)+25+(75*(i)), 213+self.popUpSize[1]/4+80,65,120)
          ...
          cRect[i].topleft = (196.5-(self.popUpSize[0]/2)+30+(75*(i)), 213+self.popUpSize[1]/4+100)
